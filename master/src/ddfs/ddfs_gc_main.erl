...
-record(state, {
          ...
          last_response_time = erlang:timestamp()          :: erlang:timestamp(),
...
handle_call({is_orphan, Type, ObjName, Node, Vol}, _, S) ->
    S1 = S#state{last_response_time = erlang:timestamp()},
...
handle_cast(start, #state{phase = start} = S) ->
    ...
    case get_all_tags() of
        {ok, Tags, OkNodes} ->
            Phase = build_map,
            Peers = start_gc_peers(OkNodes, self(), erlang:timestamp(), Phase),
...
handle_cast({retry_node, Node},
    ...
    Pid = ddfs_gc_node:start_gc_node(Node, self(), erlang:timestamp(), Phase, Mode),
    ...
    S1 = case num_pending_objects() of
             ...
             Pending ->
                 ...
                 S#state{phase = map_wait,
                         last_response_time = erlang:timestamp(),
...
handle_info(check_progress, #state{phase = Phase, last_response_time = LRT} = S)
  when Phase =:= build_map; Phase =:= map_wait; Phase =:= gc ->
    Since = timer:now_diff(erlang:timestamp(), LRT),
