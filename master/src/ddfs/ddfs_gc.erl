...
initial_wait(InitialWait) ->
    ...
    receive
        {From, _Req} ->
            From ! {ok, init_wait},
            Wait = timer:now_diff(erlang:timestamp(), Start) div 1000,
...
idle(Timeout) ->
    ...
    receive
        {From, status} ->
            From ! {ok, not_running},
            Wait = Timeout - timer:now_diff(erlang:timestamp(), Start) div 1000,
            idle(Wait);
        {From, start} ->
            From ! ok;
        _Other ->
...
start_gc_wait(Pid, Interval) ->
    ...
    receive
        ...
        {'EXIT', Other, Reason} ->
            ...
            start_gc_wait(Pid, Interval - (timer:now_diff(erlang:timestamp(), Start) div 1000));
