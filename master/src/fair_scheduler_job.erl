...
findpref(Task, NodeStats, Tasks, Nodes) ->
    LoadSorted = lists:sort([case lookup(Node, Tasks, {0, 0, []}) of
                                 {_N, Count, _TaskList} ->
                                     {Count, Load, {Url, Node}}
                             end
                             || {Load, {Url, Node}} <- NodeStats,
    ...
    case LoadSorted of
        ...
        [{_Load, _Count0, {Url, Node}}|_] ->
            ...
            {N, Count, TaskList} = lookup(Node, Tasks, {0, 0, []}),
...
-spec lookup(node(), gb_tree(), T) -> T.
lookup(Key, Tree, Default) ->
